...
if view_option == "📅 Mengikut Tarikh":
    ...
    column_order = []
    for col in ["Kenderaan", "No. Pendaftaran", "Tarikh Mula", "Tarikh Tamat", "PIC", "Lokasi", "Nota / Kegunaan",
                "Jenis Minyak", "Road Tax Expiry", "Insurance Expiry", "Puspakom Expiry"]:
    ...
    if "Kenderaan" in df.columns:
        ...
        if selected_vehicle:
            ...
            past = vehicle_data[vehicle_data["Tarikh Tamat"] < pd.Timestamp(today)].sort_values("Tarikh Tamat",
                                                                                                ascending=False)
            if not past.empty:
